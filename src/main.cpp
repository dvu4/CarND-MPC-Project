    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double v = j[1]["speed"]; //The current velocity
          ...
          *
          */
          double steer_value    = j[1]["steering_angle"]; //The current steering angle
          double throttle_value = j[1]["throttle"];       //The current throttle value
          ...
          // vars = {delta, a, predicted_x(0), predicted_y(0), ..., , predicted_x(N-1), predicted_y(N-1)}
          auto vars = mpc.Solve(state, coeffs);


          // steer_angle : delta , throttle : Acceleration/decceleration a
          ...
          // Note : vars = {delta, a, predicted_x(0), predicted_y(0), ..., , predicted_x(N-1),, predicted_y(N-1)}
          // predicted_x(0) = vars[2], predicted_x(0) = vars[4], ...
          // predicted_y(0) = vars[3], predicted_y(0) = vars[5], ...
          ...
          for (double i = 0; i < num_points; i += num_inc){
